...
void mainMenuForWomen(member* men, womenList* wList, int* countMen, int* sizePtr, member* women);
void findMatchForMen(member* men, womenList* wList, int* countMen, member* maleUser);
void findMatchForWomen(member* men, womenList* wList, int* sizePtr, member* women);
int matcHobbies(member* maleUser, member* women);
void printHobbies(char hobbie);
void hobbieCases(int temp);
member* newMember1(womenList* wList, char* id, char* firstName, char* lastName, int age, char sex,
	char* userName, char* password, char* description, char hobbies);
...
member addNewMen(member* men, char* idNew, char* firstNameNew, char* lastNameNew, int ageNew,
	char sexNew, char* userNameNew, char* passwordNew, char* descriptionNew, char hobb);
...
int main()
{
	...
	//the following int variable represent the counter of the men
	int *sizePtr;
	int j = 0;
	sizePtr = &j;
	int loginValue = 0;
...
member* readText(member* men, womenList* wList, int* countMen, int* sizePtr, int j)
{
	...
	while (!feof(fp))
	{
		...
		//list[numOfLine].hobbies = (char*)malloc(strlen(hob) + 1);
...
*************************************************************************/
member* newMember1(womenList* wList, char* id, char* firstName, char* lastName, int age,
	char sex, char* userName, char* password,
	char* description, char hobbies)
{
	//fill all the struct member fields to a new women member 
	...
	if (woman->hobbies == NULL)
	{
		printf("Allocation is failed\n");
		exit(1);
	}
	*(woman->hobbies) = hobbies;
	//return the new woman
	return woman;
}
...
int login(member* men, womenList* wList, int* countMen, int *sizePtr)
{
	...
	scanf("%s", &userNameTMP);
	//the following loop check if the male username is exist
	for (i = 0; i < *sizePtr; i++)
	{
		if (strcmp(men[i].userName, userNameTMP) == 0)
		{
			//it's a man so i switch flag's value to 1
			flag = 1;
			printf("Please enter your password:\n");
			scanf("%s", &passwordTMP);
			if (strcmp(men[i].password, passwordTMP) == 0)
			{
				printf("Hello %s!\n", men[i].firstName);
			...
			if (strcmp(men[i].password, passwordTMP) != 0)
			{
				//flag keeps being the same value because it's a male user
				flag = 1;
		...
		if (strcmp(men[i].userName, userNameTMP) != 0)
		{
			//maybe it's a woman so i change the variable of flag 
			flag = 0;
			//continue to the next men
			continue;
		}
	}
	...
	if (flag == 0)
	{
		...
		while (iterator != NULL)
		{
			if (strcmp(iterator->data->userName, userNameTMP) == 0)
			{
				//it's a woman
				flag = 0;
				printf("Please enter your password:\n");
				scanf("%s", &passwordTMP);
				if (strcmp(iterator->data->password, passwordTMP) == 0)
				{
					printf("Hello %s!\n", iterator->data->firstName);
					mainMenuForWomen(men, wList, countMen, sizePtr, iterator->data);
					return 0;
				}
				if (strcmp(iterator->data->password, passwordTMP) != 0)
				{
			...
			if (strcmp(iterator->data->userName, userNameTMP) != 0)
			{
				//maybe it's a man so i change the variable to flag = 1
				flag = 1;
			}
		...
		if (option == 2)
		{
			...
			for (i = 0; i < *sizePtr; i++)
			{
				if (strcmp(men[i].id, maleUser->id) == 0)
				{
					...
					/*temp = (member*)realloc(men, sizeof(member)*((*sizePtr) - 1));
					if (temp == NULL)
					{
						exit(1);
					}
...
void mainMenuForWomen(member* men, womenList* wList, int* countMen, int* sizePtr, member* women)
{
	...
	while (option != 3)
	{
		...
		if (option == 1)
		{
			findMatchForWomen(men, wList, sizePtr, women);
		}
		//delete the women
		if (option == 2)
		{
			//deleteWomen(men, wList, &countMen, sizePtr, iterator);
			deleteWoman(wList, women);
			break;
		}
		//logout
		if (option == 3)
...
void findMatchForMen(member* men, womenList* wList, int* countMen, member* maleUser)
{
	...
	while (iterator != NULL)
	{
		...
		if (iterator->data->age >= fromAge && iterator->data->age <= toAge)
		{
			...
			match = matcHobbies(maleUser, iterator->data);
			if (match == 1)
			{
...
int matcHobbies(member* maleUser, member* women)
{
	...
	char womenHobbie = *(women->hobbies);
...
member* newMember2(int* sizePtr, int* countMen, member* men, womenList* wList)
{
	...
	int ageNew;
	char sexNew;
	char idNew[10], firstNameNew[15], lastNameNew[15], userNameNew[15], passwordNew[15], descriptionNew[211];
...
/************************************************************************
* function name:  newWomenList
* The Input: none.
* The output: none it's a void function.
* The Function operation: the function creating the women linked-list
and the head of the list.
*************************************************************************/
//creating a new womenlist
womenList* newWomenList()
{
	//allocating in order to create the women list
	...
	//allocation for the list's head
...
/************************************************************************
* function name: add
* The Input: wList - the women linked list and the required woman
* The output: none it's a void function.
* The Function operation: the function adding a woman to the end of the list
*************************************************************************/
...
/************************************************************************
* function name: push
* The Input: wList - the women linked list and the required women 
* The output: none it's a void function.
* The Function operation: the function call to the function add in order to
add a new woman to the begining of the list.
*************************************************************************/
...
* The Input: wList - the women linked list
* The output: the next womenItem in the linked-list
* The Function operation: it returns the next womenItem in the linked-list.
*************************************************************************/
womenItem* newIterator(womenList* wList)
{
...
/************************************************************************
* function name: printWomenList
* The Input: wList - the women linked list
* The output: none it's a void function.
* The Function operation: the function call to the function which print the
members. in this case the women list's members.
*************************************************************************/
void printWomenList(womenList* wList)
{
	womenItem* iterator = newIterator(wList);
...
{
 printf("%s %s %s %d %c %s %s %s %s", pm->id, pm->firstName, pm->lastName, pm->age, pm->sex,
	 pm->userName, pm->password, pm->description, pm->hobbies);
}
...
* The Input: the women linked-list, the men's array and the men's counter.
* The output: none it's a void function.
* The Function operaton: the function prints  the details of each member
- male and female, which appeared in the program.
*************************************************************************/
void outPutText(womenList* wList, member* men, int* sizePtr)
{
	int i = 0;
...
/************************************************************************
* function name: newMen
* The Input: the mens dynamic array and all the member's fields
* The output: return the new member (male)
* The Function operation: i call this function when the new member is male.
this function fill all the member's field and return the new male member to the 
men's array.
*************************************************************************/
member addNewMen(member* men, char* idNew, char* firstNameNew, char* lastNameNew, int ageNew,
	char sexNew, char* userNameNew, char* passwordNew, char* descriptionNew, char hobb)
...
/************************************************************************
* function name: freeWomenList
* The Input: the women-linked list
* The output: none it's a void function.
* The Function operaton: i call this function when i exit
the whole program. this function free all the linked-list of the women.
*************************************************************************/
void freeWomenList(womenList* wList)
{
	//free the head of the least
...
/************************************************************************
* function name: freeWomenItemLast
* The Input: a women item
* The output: none it's a void function (recursion function).
* The Function operaton: i call this function when i exit the program,
this function free any member which is a woman by recursion calling.
*************************************************************************/
void freeWomenItemLast(womenItem* item)
{
	if (item != NULL)
	{
		//call the function free member
		freeMember(item->data);
		free(item->data);
		//recursion function
...
/************************************************************************
* function name: freeDynamicMen
* The Input: the men's array and men's counter.
* The output: none it's a void function.
* The Function operaton: it frees any field of each male which appreas in 
the men array while he isn't NULL.
*************************************************************************/
member* freeDynamicMen(member* men, int* sizePtr)
{
	int i = 0;
...
* The Input: womenList* wList (the linked list), member* theRequiredUser(a women member which we want to
* delete).
* The output: none its a void function.
* The Function operation: delete the required woman from the linked list
*************************************************************************/
void deleteWoman(womenList* wList, member* woman)
{
...
/************************************************************************
* function name: freeWomanItem
* The Input: womenitem* - iterator (the required woman)
* The output: none its a void function.
* The Function operation: the function free the memory's allocation of any 
field which was allocated in the program.
*************************************************************************/
void freeWomanItem(womenItem* iterator)
{
